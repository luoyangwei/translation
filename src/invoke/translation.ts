...
export class YoudaoTransilation implements Translator {
    readonly requestUrl: string = "https://dict.youdao.com/suggest?num=5&ver=5.0&doctype=json&cache=false&le=en&q=";

    getChannel(): string {
        return "youdao";
    }

    async translate(text: string, configuration?: Configuration): Promise<string> {
...
export class BaiduTransilation implements Translator {
    readonly requestUrl: string = "https://fanyi-api.baidu.com/api/trans/vip/translate";
    ...
    async translate(text: string, configuration?: Configuration): Promise<string> {
        ...
        let response: any = await got.post(this.requestUrl, {
            headers: {
            ...
            form: {
                ...
                "salt": salt,
                "sign": md5(configuration?.appid + text + salt + configuration?.secretKey)
            }
        }).json();
